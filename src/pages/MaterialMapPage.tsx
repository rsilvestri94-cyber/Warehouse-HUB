...
export function MaterialMapPage() {
    ...
    const [highlightMaterial, setHighlightMaterial] = useState<
        string | undefined
    >(undefined);
    ...
    const handleLocate = (row: MaterialRow) => {
        ...
        const slotKey =
            parsed.kind === "triple"
                ? `P${parsed.position}`
                : parsed.kind === "bluebox"
                  ? `BOX${String(parsed.boxNumber).padStart(2, "0")}`
                  : "single";
    ...
    const selectedRackModel = selectedRack
        ? (warehouse.get(selectedRack) ?? {
              rack: selectedRack,
              levels: {},
              itemCount: 0,
          })
        ...
        <div className="min-h-screen bg-grey-bg">
            <header className="border-b border-grey-line bg-white px-4 py-4 sm:px-6">
                <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2">
                    <div>
                        <h1 className="font-heading text-lg font-bold text-blue-dark">
                            Mappa Materiali
                        </h1>
                        <p className="text-xs text-ink/50">
                            Localizzazione scaffali e ricerca materiali a
                            magazzino
                        </p>
                    </div>
                    <Link
                        to="/"
                        className="rounded-full border border-grey-line px-3 py-1.5 text-xs font-semibold text-blue-mid hover:bg-grey-bg"
                    >
                        ← Torna all'hub
                    </Link>
                    <button
                        onClick={() => window.location.reload()}
                        className="flex items-center gap-2 rounded-full border border-grey-line px-3 py-1.5 text-xs font-semibold text-blue-mid hover:bg-grey-bg cursor-pointer"
                    >
                        <RefreshCcw width={16} />
                        Aggiorna Dati
                    </button>
            ...
            <main className="mx-auto flex max-w-7xl flex-col gap-5 px-4 py-6 sm:px-6">
                ...
                {!error && rows === null && (
                    <div className="rounded-2xl border border-grey-line bg-white p-6 text-center text-sm text-ink/50">
                        Caricamento…
                    </div>
                )}
                                ...
                                {selectedRackModel ? (
                                    <>
                                        <div className="font-heading text-xs font-bold uppercase tracking-wide text-blue-mid">
                                            ...
                                            {selectedRackModel.itemCount ===
                                                0 && (
                                                <span className="ml-2 font-normal normal-case text-ink/40">
                                                    — nessun materiale mappato
                                                </span>
                                            )}
                                        </div>
                                        <RackElevation
                                            rack={selectedRackModel}
                                            selectedSlotKey={
                                                selectedLevel && selectedSlot
                                                    ? `${selectedLevel}-${selectedSlot.key}`
                                                    : null
                                            }
                                    ...
                                    <div className="rounded-2xl border border-dashed border-grey-line p-6 text-center text-sm text-ink/40">
                                        Seleziona uno scaffale sulla mappa per
                                        vedere il prospetto verticale.
                                    </div>
                                )}
                            </div>
                        </div>
